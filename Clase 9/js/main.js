...
const procesoElectoral2012 = new ProcesoElectoral(presupuestoPrevio[0], presupuestoActual[0], presupuestoPartidos[0], votos[0], listaNominal[0]);

//OBJETO procesoElectoral2015

const procesoElectoral2015 = new ProcesoElectoral(presupuestoPrevio[1], presupuestoActual[1], presupuestoPartidos[1], votos[1], listaNominal[1]);

//OBJETO procesoElectoral2018

const procesoElectoral2018 = new ProcesoElectoral(presupuestoPrevio[2], presupuestoActual[2], presupuestoPartidos[2], votos[2], listaNominal[2]);

//OBJETO procesoElectoral2021

const procesoElectoral2021 = new ProcesoElectoral(presupuestoPrevio[3], presupuestoActual[3], presupuestoPartidos[3], votos[3], listaNominal[3]);
...
botonCalcular.onclick = (event) => {
  ...
  if (document.getElementById("input-fecha").value == procesosElectorales[0]) {
    mensaje.innerHTML = `<h3>Proceso Electoral ${procesosElectorales[0] - 1}-${
      procesosElectorales[0]
    }</h3><p>Con una participación ciudadana de <b>${procesoElectoral2012.participacion()}%</b> en la elección de <b>${
      procesosElectorales[0]
    }</b>, el costo de cada voto fue de <b>${procesoElectoral2012.costoVoto()} pesos</b></p><br><img src="./media/voto-back.png" alt="voto"><button class="btn btn-secondary" data-bs-dismiss="offcanvas" type="button">Cerrar</button>`;
  } else if (document.getElementById("input-fecha").value == procesosElectorales[1]) {
    mensaje.innerHTML = `<h3>Proceso Electoral ${procesosElectorales[1] - 1}-${
      procesosElectorales[1]
    }</h3><p>Con una participación ciudadana de <b>${procesoElectoral2015.participacion()}%</b> en la elección de <b>${
      procesosElectorales[1]
    }</b>, el costo de cada voto fue de <b>${procesoElectoral2015.costoVoto()} pesos</b></p><br><img src="./media/voto-back.png" alt="voto"><button class="btn btn-secondary" data-bs-dismiss="offcanvas" type="button">Cerrar</button>`;
  } else if (document.getElementById("input-fecha").value == procesosElectorales[2]) {
    mensaje.innerHTML = `<h3>Proceso Electoral ${procesosElectorales[2] - 1}-${
      procesosElectorales[2]
    }</h3><p>Con una participación ciudadana de <b>${procesoElectoral2018.participacion()}%</b> en la elección de <b>${
      procesosElectorales[2]
    }</b>, el costo de cada voto fue de <b>${procesoElectoral2018.costoVoto()} pesos.</b></p><br><img src="./media/voto-back.png" alt="voto"><button class="btn btn-secondary" type="button" data-bs-dismiss="offcanvas">Cerrar</button>`;
  } else if (document.getElementById("input-fecha").value == procesosElectorales[3]) {
    mensaje.innerHTML = `<h3>Proceso Electoral ${procesosElectorales[3] - 1}-${
      procesosElectorales[3]
    }</h3><p>Con una participación ciudadana de <b>${procesoElectoral2021.participacion()}%</b> en la elección de <b>${
      procesosElectorales[3]
    }</b>, el costo de cada voto fue de <b>${procesoElectoral2021.costoVoto()} pesos.</b></p><br><img src="./media/voto-back.png" alt="voto"><button class="btn btn-secondary" type="button" data-bs-dismiss="offcanvas">Cerrar</button>`;
  } else {
    mensaje.innerHTML = `No seleccionaste una opción válida.<br>Las opciones disponibles son: <b>${procesosElectorales.join(
      "-"
    )}</b><br><img src="./media/voto-back.png" alt="voto"><button class="btn btn-secondary" type="button" data-bs-dismiss="offcanvas">Cerrar</button>`;
  }
};
...
botonVariables.onclick = (event) => {
  ...
  if (document.getElementById("input-fecha").value == procesosElectorales[0]) {
    variables.innerHTML = `<h3>Variables ${procesosElectorales[0] - 1}-${procesosElectorales[0]}</h3><ul><li>Presupuesto del ${
      procesosElectorales[0] - 1
    } para elecciones = <b>$${new Intl.NumberFormat().format(presupuestoPrevio[0].value)}</b></li>
    ...
    variables.innerHTML = `<h3>Variables ${procesosElectorales[1] - 1}-${procesosElectorales[1]}</h3><ul><li>Presupuesto del ${
      procesosElectorales[1] - 1
    } para elecciones = <b>$${new Intl.NumberFormat().format(presupuestoPrevio[1].value)}</b></li>
    ...
    variables.innerHTML = `<h3>Variables ${procesosElectorales[2] - 1}-${procesosElectorales[2]}</h3><ul><li>Presupuesto del ${
      procesosElectorales[2] - 1
    } para elecciones = <b>$${new Intl.NumberFormat().format(presupuestoPrevio[2].value)}</b></li>
    ...
    variables.innerHTML = `<h3>Variables ${procesosElectorales[3] - 1}-${procesosElectorales[3]}</h3><ul><li>Presupuesto del ${
      procesosElectorales[3] - 1
    } para elecciones = <b>$${new Intl.NumberFormat().format(presupuestoPrevio[3].value)}</b></li>
    ...
    variables.innerHTML = `No seleccionaste una opción válida.<br>Las opciones disponibles son: <b>${procesosElectorales.join(
      "-"
    )}</b><br><img src="./media/voto-back.png" alt="voto"><button class="btn btn-secondary" type="button" data-bs-dismiss="offcanvas">Cerrar</button>`;
  }
};
...
function menos() {
  ...
  if (document.getElementById("input-fecha").value == procesosElectorales[0]) {
    ...
    document.getElementById("participacion").innerHTML = `Con una participación de <b>${currentVal}%</b> en las elecciones de <b>${
      document.getElementById("input-fecha").value
    }</b>, el costo de cada voto habría sido de <b>$${calculo.value} </b>`;
  } else if (document.getElementById("input-fecha").value == procesosElectorales[1]) {
...
const calculoPersonal = new ProcesoElectoralPersonal(inputPersonalPrevio, inputPersonalActual, inputPersonalPartidos, inputPersonalVotos, inputPersonalLista);
...
function borrarStorage() {
  if (sessionStorage.getItem("inputProcesoElectoralPersonal") != null) sessionStorage.removeItem("inputProcesoElectoralPersonal");
}
...
formCalculoPersonalizado.addEventListener("submit", (event) => {
  ...
  const outputCalculoPersonal = new ProcesoElectoralPersonal(inputPersonalPrevio, inputPersonalActual, inputPersonalPartidos, inputPersonalVotos, inputPersonalLista);
  ...
  mostrarCalculoPersonal.innerHTML = `<h3>Cálculo Personalizado</h3><p>Con una participación ciudadana de <b>${participacionPersonal()}%</b> y un presupuesto total de <b>${new Intl.NumberFormat().format(
    presupuestoEleccionesPersonal()
  )} pesos</b>, el costo de cada voto sería de <b>${new Intl.NumberFormat().format(costoVotoPersonal())} pesos</b></p>
